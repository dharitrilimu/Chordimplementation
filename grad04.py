        ...
        while True:
            ...
            if nodevalues[0]=='JOIN':
                ...
                if frompeerid==0:
                    ...
                    print("file stored in grad04!!!Request generator grad01")
                elif frompeerid==1:
                    peer1files.append(nodevalues[2])
                    filestore.update({1:peer1files})
                    print("file stored in grad04!!!Request generator grad02")
                elif frompeerid==2:
                    peer2files.append(nodevalues[2])
                    filestore.update({2:peer2files})
                    print("file stored in grad04!!!Request generator grad03")
                elif frompeerid==3:
                    peer3files.append(nodevalues[2])
                    filestore.update({3:peer3files})
                    print("file stored in grad04!!!Request generator grad04")
                else:
                    peer4files.append(nodevalues[2])
                    filestore.update({4:peer4files})
                    print("file stored in grad04!!!Request generator grad05")
                print("file stored in peer4 as tuple")                   
                print(filestore[2])            
                c.sendall(("file "+nodevalues[2] +" is stored successfully in grad04 ").encode())               
            else:
                print("no request")
roothost='10.0.130.21'                 
hostname='10.0.130.24'
hostport=12111
...
def leaveChord():
    ...
    response=updateroot()
    node3=node('',0,hostname,hostport,'',0,3)
    node3.printNode()
    print(response)
def fileRandomStore():
    starttime=time.time()
...
def joinChord():
    if hostname == '10.0.130.21':
        ...
        node3=node('',0,hostname,hostport,'',0,3)
        joinrequest='JOIN|'+str(node3.id)+'|'+node3.nn+'|'+str(node3.np)
        responsestring=sendRequest(roothost, hostport, joinrequest)
        ...
        if (data2[0]=='PREDSUCC') and (data2[2]=='one'):
            ...
            node3.printNode()
            print("inside many")
            data=updatepredecessor(node3.nn,node3.pn)
            print(data)
            print("update successor")
            updatesuccessor(node3.nn,node3.sn)
            print("inside many") 
