    ...
    def listensocket(self):
        ...
        while True:
            ...
            if nodevalues[0]=='JOIN':
                ...
                print("file stored in peer3 as tuple")                   
                print(filestore[2])            
                c.sendall(("file "+nodevalues[2] +" is stored successfully in grad03 ").encode())               
            else:
                print("no request")
roothost='10.0.130.21'                 
hostname='10.0.130.23'
hostport=12111
...
def leaveChord():
    ...
    response=updateroot()
    node3=node('',0,hostname,hostport,'',0,2)
    node3.printNode()
    print(response)
def fileRandomStore():
    starttime=time.time()
